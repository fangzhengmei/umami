...
**事件类型过滤说明** (`event_type NOT IN (2, 5)`):

根据 `EVENT_TYPE` 常量定义：
| 事件类型 | event_type | 是否统计 | 说明 |
|---------|-----------|---------|------|
| pageView | 1 | ✅ 包含 | 页面浏览 |
| customEvent | 2 | ❌ 排除 | 自定义事件 |
| linkEvent | 3 | ✅ 包含 | 链接点击事件 |
| pixelEvent | 4 | ✅ 包含 | Pixel 追踪事件 |
| performance | 5 | ❌ 排除 | 性能指标事件 |

**统计范围总结**：包含页面浏览（type=1）、链接点击事件（type=3）、Pixel 追踪事件（type=4）三类，排除自定义事件和性能事件。
